...
LARGER0_ARGS = [
    "../inputs/larger0/bin/m0.bin",
    "../inputs/larger0/bin/m1.bin",
...
class TestReadMatrix(TestCase):
    ...
    def test_simple0(self):
        self.doReadMatrix(
            input_file="inputs/simple0/bin/inputs/input2.bin",
            result_row=3,
            result_col=1,
            result_array=[2, 1, 6],
        )
    ...
    def test_larger0(self):
        self.doReadMatrix(
            input_file="inputs/larger0/bin/inputs/input0.bin",
            result_row=15,
            result_col=15,
            result_array=[
                -5,
...
class TestClassify(TestCase):
    ...
    def test_simple0_input0(self):
        self.run_classify(
            input_dir="inputs/simple0/bin", input="input0.bin", output_id=0
        )

    def test_simple0_input0_print_out(self):
        self.run_classify(
            input_dir="inputs/simple0/bin", input="input0.bin", output_id=0, msg="2"
        )

    def test_fail_malloc(self):
        # unfortunately this test actually does not fail inside classify, but inside read_matrix
        self.run_classify(
            input_dir="inputs/simple0/bin",
            input="input0.bin",
            output_id=0,
            fail="malloc",
            code=88,
...
def compare_files(test, actual, expected):
    ...
    test.assertTrue(
        os.path.isfile(actual),
        f"It seems like the program never created the output file {actual}",
    )
...
class TestMain(TestCase):
    ...
    def run_main(self, inputs, output_id, lbl):
        ...
        compare_files(
            self,
            actual=script_dir / ".." / args[-1],
            expected=script_dir / ".." / reference,
        )
    ...
    def test_chain_main(self):
        ...
        compare_files(
            self,
            actual=script_dir / ".." / outfile_simple,
            expected=script_dir / ".." / reffile_simple,
        )
        compare_files(
            self,
            actual=script_dir / ".." / outfile_larger,
            expected=script_dir / ".." / reffile_larger,
        )
        self.assertEqual(
            stdout,
            "Two classifications:\n2\n2\nTwo classifications:\n2\n48\nTwo classifications:\n48\n48",
