...
map:
    ...
    ### YOUR CODE HERE ###
    addi sp, sp, -8
    sw s0, 0(sp) # Save the head of the linked list
    ...
    # THINK: Why a0?
    ### YOUR CODE HERE ###
    lw a0, 0(s0) # a0 is the first argument of the callee

    # Call the function in question on that value. DO NOT use a label (be prepared to answer why).
    # Hint: Where do we keep track of the function to call? Recall the parameters of "map".
    ### YOUR CODE HERE ###
    jalr ra, s1, 0 # Using jalr instead of jal makes the code more robust

    # Store the returned value back into the node
    # Where can you assume the returned value is?
    ### YOUR CODE HERE ###
    sw a0, 0(s0) # Returned value is stored in a0 like %rax in x86-64

    # Load the address of the next node into a0
    # The address of the next node is an attribute of the current node.
    # Think about how structs are organized in memory.
    ### YOUR CODE HERE ###
    lw a0, 4(s0) # The pointer to the next node should have higher address which could be figured out by examining the create_default_list function
    

    # Put the address of the function back into a1 to prepare for the recursion
    # THINK: why a1? What about a0?
    ### YOUR CODE HERE ###
    add a1, s1, x0 # a0 is the first argument and a1 is the second
